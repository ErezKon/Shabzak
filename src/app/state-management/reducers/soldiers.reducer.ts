...
export const soldiersReducer = createReducer(
    ...
    on(soldierActions.getSoldiers, (state) => ({ ...state, loading: true })),
    on(soldierActions.getSoldiersSuccess, (state, {soldiers}) => ({ ...state, soldiers: soldiers, loading: false })),
    on(soldierActions.getSoldiersFailure, (state) => ({ ...state, loading: false })),
